...
*   **`browser_utils.py` (环境工厂)**
    ...
    *   通过 owner-only 权限和 Profile 锁，防止其他系统用户读取 Cookie，并阻止多个 Agent 并发打开同一 Profile。
...
3.  **提示 `profile_locked`？**
    *   表示另一个 Agent 正在使用共享 Chrome Profile。结束另一个发布任务后重试，不要直接杀死日常 Chrome 进程。
...
│   ├── profile_lock.py         # 共享 Profile 并发锁
│   └── run.py                  # 运行入口
├── README.md                   # 本文档
